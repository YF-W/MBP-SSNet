...
class DetailMega(nn.Module):
    # 初始化
    def __init__(self, in_channels, out_channels):  #
        super(DetailMega, self).__init__()

        self.DCF=ConvF(in_channels,out_channels)  # 5.1.2
    ...
    def forward(self, x,y):
        ...
        x_2_p=self.A2(torch.cat((x_2, x_1), dim=1))
...
class DNET(nn.Module):
    def __init__(self, features=[64, 128, 256, 512], out_channels=1):
        ...
        self.DetailMega64_128=DetailMega(64,128)
        self.DetailMega128_256 = DetailMega(128,256)
        self.DetailMega256_512 = DetailMega(256,512)
    ...
    def forward(self, x):
        ...
        xe1_p=self.pool(xe1)
        xe2_p1= torch.cat((v3_3, e1), dim=1)  #
        xe2_p2=self.conv64(xe2_p1)  # ([4, 64, 112, 112])
        xe2=self.DetailMega64_128(xe1_p,xe2_p2)
        xe2_p = self.pool(xe2)

        xe3_p1 = torch.cat((v2_2, e2), dim=1)  # 65+128
        xe3_p2 = self.conv128(xe3_p1)
        xe3 = self.DetailMega128_256(xe2_p,xe3_p2)
        xe3_p = self.pool(xe3)
