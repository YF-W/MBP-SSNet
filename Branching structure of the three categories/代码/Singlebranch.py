...
class MPDFR(nn.Module):
    ...
    def forward(self, q, m1, m2, x1, x2, y):
        y1 = self.A(y)
...
class MPBFE(nn.Module):

    def __init__(self, in_channels, out_channels):
        super(MPBFE, self).__init__()
    ...
    def forward(self, x):
        x_1 = self.A1(x)
        x_2 = self.DCN(self.DCS(x))
        x_2_p = torch.cat((x_2, x_1), dim=1)
        x_3 = self.dconv(self.DW(x))
        x_out = x_3 + x_2_p

        return x_out


class MBP_SSNet(nn.Module):
    def __init__(self, features=[64, 128, 256, 512], out_channels=1):
        super(MBP_SSNet, self).__init__()
        ...
        self.DetailMega64_128 = MPBFE(64, 128)
        self.DetailMega128_256 = MPBFE(128, 256)
        self.DetailMega256_512 = MPBFE(256, 512)

        self.vitLayer_UpConv = nn.ConvTranspose2d(65, 65, kernel_size=2, stride=2)

        self.block64_64 = MPDFR(64, 64, 64)
    ...
    def forward(self, x):
        ...
        xe2 = self.DetailMega64_128(xe1_p)  # xe2 torch.Size([4, 128, 112, 112])
        xe2_p = self.pool(xe2)

        # xe3_p1 = torch.cat((v2_2, e2), dim=1)  # 65+128
        # xe3_p2 = self.conv128(xe3_p1)
        xe3 = self.DetailMega128_256(xe2_p)  # ([4, 256, 56, 56])
        xe3_p = self.pool(xe3)

        q = self.up256_128(xe3) + xe2

        # xe4_p1 = torch.cat((v1_1, e3), dim=1)  # 65+256
        # xe4_p2 = self.conv256(xe4_p1)
        xe4 = self.DetailMega256_512(xe3_p)
        xe4_p = self.pool(xe4)
        ...
        xj2_p = self.up256_256(xu3_1)  # 56*2=112
        xj2 = torch.cat((xu2_1, xj2_p), dim=1)  # 128+256---------------------------2
        ...
        xj4 = torch.cat((xu3_2, xj2), dim=1)  # 128+128--------------4
        ...
        x = self.block64_64(q, xj6, xj3, xj7, xj5, xj4)
...
# model = MBP_SSNet(features=[64, 128, 256, 512], out_channels=1)
# print(model(x).shape)

net = MBP_SSNet(features=[64,128,256,512],out_channels=1)  # 定义好的网络模型
